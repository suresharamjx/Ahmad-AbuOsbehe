...
class Interface {
  ...
  async read(barberID) {
    if (barberID) {
      const sql = `SELECT barber.working_hours,tickets.id ,tickets.barber_id, tickets.time, client.profile_pic, client.user_name, client.phone_num,services.service_name, services.price, services.estimated_time,services.id as service_id,client.id as client_id
      FROM tickets
      INNER JOIN client ON client.id=tickets.client_id 
      INNER JOIN services ON services.id=tickets.service_id 
      INNER JOIN barber ON barber.id=tickets.barber_id 
      
      WHERE tickets.barber_id =$1;`;
